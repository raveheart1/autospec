  ...
  autospec analyze "Verify API contracts"`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		notifyAndReturn := func(err error) error {
			...
			notifHandler.OnCommandComplete("analyze", false, duration)
			return err
		}
		...
		execErr := orch.ExecuteAnalyze(specName, prompt)

		// Calculate duration and send command completion notification
		duration := time.Since(startTime)
		success := execErr == nil
		notifHandler.OnCommandComplete("analyze", success, duration)

		if execErr != nil {
			return fmt.Errorf("analyze stage failed: %w", execErr)
		}
