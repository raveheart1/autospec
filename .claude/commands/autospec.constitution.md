...
1. **Get version info**: Source the common script to get autospec version and timestamp:

   ```bash
   source .autospec/scripts/common.sh && echo "AUTOSPEC_VERSION=$(get_autospec_version)" && echo "CREATED_DATE=$(get_iso_timestamp)"
   ```
