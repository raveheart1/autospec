...
### Changed
- **BREAKING**: DAG commands now use workflow file path (e.g., `dag merge workflow.yaml`) with state stored inline in dag.yaml; automatic migration from legacy state files
- Renamed `autospec dag` command to `autospec waves` for task execution wave visualization

### Removed
- **BREAKING**: `dag resume` and `dag retry` commands removed - functionality merged into idempotent `dag run`

### Fixed
- Data races in parallel executor state management
